...
titanic$AgeCat[titanic$Age>=65] <- "above 65"
...
write.csv (titanic, file = "C:/UTD/Semester 2 - S20/Data Viz/Assignment/Kmeans/titanicNew.csv")
...
tree_model = rpart(Survived ~ Pclass + Sex + AgeCat + SibSpCat + ParchCat, data =  training_data, method = "class", control = rpart.control(minsplit=10, cp=0.00))
...
titanicNew<-read.csv("C:/UTD/Semester 2 - S20/Data Viz/Assignment/Kmeans/titanicNew.csv")
...
write.csv(titanicUpdated,file = "C:/UTD/Semester 2 - S20/Data Viz/Assignment/Kmeans/titanicUpdated.csv")

####

titanic.scaled <- scale(data.frame(titanicNew$Age,titanicNew$Parch,titanicNew$SibSp,titanicNew$Fare))
